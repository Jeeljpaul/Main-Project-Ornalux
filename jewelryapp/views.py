...
def register(request):
    if request.method == 'POST':
        ...
        # Initialize errors dictionary for field-specific errors
        errors = {}
        ...
        if len(password) < 8 or not any(char.isdigit() for char in password) or not any(char.isalpha() for char in password) or not any(char in '@$!%*#?&' for char in password):
            errors['password'] = ['Password must be at least 8 characters long, include a letter, a number, and a special character.']
